...
BACKUP_POD_HOSTIP=$(kubectl get pods -n px-backup -l app=px-backup -o jsonpath='{.items[*].status.hostIP}' 2>/dev/null)
...
pxbackupctl login -s http://$BACKUP_POD_HOSTIP:$backupPort -u admin -p admin
